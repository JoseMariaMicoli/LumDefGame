class LumDefWeapon extends Weapon;
...
reliable client function ClientGivenTo(Pawn NewOwner, bool bDoNotActivate)
{
	...
	{
		// Cast the new owner into a LumDefPawn as we need the weapon socket name
		// If the cast succeeds, we check that the new owner's mesh has a socket by that name
		LumDefPawn = LumDefPawn(NewOwner);
		if (LumDefPawn != None && NewOwner.Mesh.GetSocketByName(LumDefPawn.WeaponSocketName) != None)
		{
			...
			Mesh.SetLightEnvironment(LumDefPawn.LightEnvironment);
			// Attach the weapon mesh to the new owner's skeletal meshes socket
			NewOwner.Mesh.AttachComponentToSocket(Mesh, LumDefPawn.WeaponSocketName);
		}
	}
}
